...
public class IMDbMovieController {
    ...
    @GetMapping("/getMovieByRating/{ratingValue}")
    public ResponseEntity<List<IMDbMovie>> findMovieByRating(@PathVariable("ratingValue") String ratingValue){
        List<IMDbMovie> imdbMovies;
        imdbMovies = imDbMovieService.findMovieByRatingList(ratingValue);
        return ResponseEntity.ok(imdbMovies);
    }
